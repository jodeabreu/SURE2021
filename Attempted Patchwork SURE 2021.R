...
Fungus_phylums= ggplot(stonehill_tax %>% filter(quality_grade != 'needs_id') %>% filter(!is.na(phylum)) %>% filter(kingdom=="Fungi")) +
...
Animal_phylums= ggplot(stonehill_tax %>% filter(quality_grade != 'needs_id') %>% filter(!is.na(phylum)) %>% filter(kingdom=="Animalia")) +
  geom_bar(aes(x=phylum, fill=phylum)) +
  facet_wrap(~kingdom, scales='free_x') + 
  theme_light()+ 
  theme(axis.text.x= element_text(angle=50, vjust= 1.2, hjust=1.2 )) +
  add_phylopic(Arthropods, alpha = 1, x = .9, y = 310, ysize = 80, color = "yellow") + 
  add_phylopic(Chordata, alpha = 1, x = 2, y = 550, ysize = 35, color = "red") +
  add_phylopic(Mollusca, alpha = 1, x = 3, y = 120, ysize = 75, color = "blue") +
  ylim(0,600) +
  ylab("Number of Observations") +
  scale_fill_manual(values = c("yellow", "red", "blue"))+
  theme(legend.position='none')

Plant_phylums= ggplot(stonehill_tax %>% filter(quality_grade != 'needs_id') %>% filter(!is.na(phylum)) %>% filter(kingdom=="Plantae")) +
  geom_bar(aes(x=phylum, fill=phylum)) +
  scale_fill_manual(values = c("green4", "green"))+
  facet_wrap(~kingdom, scales='free_x') + 
  theme_light()+
  theme(axis.text.x= element_text(angle=50, vjust= 1.2, hjust=1.2 )) +
  add_phylopic(Bryophyta, alpha = 1, x = 1, y = 100, ysize = 130, color = "green4") +
  add_phylopic(Tracheophyta, alpha = 1, x = 2, y = 510, ysize = 130, color = "green") +
  theme(legend.position='none') +
  ylim(0,600) +
  ylab("") 
...
stonehill_tax %>% 
  filter(kingdom=="Plantae") %>% filter(quality_grade != 'needs_id') %>%
  group_by(phylum) %>%
  summarise(count=n()) %>%
  arrange(desc(count))

stonehill_tax %>% 
  filter(kingdom=="Fungi") %>% filter(quality_grade != 'needs_id') %>%
  group_by(phylum) %>%
  summarise(count=n()) %>%
  arrange(desc(count))

stonehill_tax %>% 
  filter(kingdom=="Animalia") %>% filter(quality_grade != 'needs_id') %>%
  group_by(phylum) %>%
  summarise(count=n()) %>%
  arrange(desc(count))
