...
stonehill_tax = bind_cols(stonehill, taxonomy)
 ...
 ggplot(stonehill_tax %>% filter(phylum== "Arthropoda"))+
  geom_bar(aes(x=order)) +
  facet_wrap(~kingdom, scales='free_x') +
 theme(axis.text.x= element_text(angle=50, vjust= 1, hjust=1.2 )) +
 add_phylopic(Lepidoptera, alpha= 1, x=12, y=115, ysize=10, color = "black") +
add_phylopic(CommonEasternBumbleBee, alpha= 1, x=8, y=60, ysize=15, color = "black") +
add_phylopic(Odonata, alpha= 1, x=15, y=40, ysize=15, color = "black") +
add_phylopic(Diptera, alpha= 1, x=5.1, y=40, ysize=20, color = "black") + 
add_phylopic(Coleoptera, alpha= 1, x=4, y=40, ysize=20, color = "black")+
